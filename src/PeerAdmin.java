...
public class PeerAdmin {
    ...
    public synchronized void updateDownloadRate(String endpeerid) {
        int current = this.downloadRate.getOrDefault(endpeerid, 0);
        this.downloadRate.put(endpeerid, current + 1);
    }
    ...
    public synchronized void closeHandlers() {
        for (String peer : this.joinedThreads.keySet()) {
            Thread t = this.joinedThreads.get(peer);
            if (t != null && t.isAlive()) {
                // Modern Java: don't use stop(); interrupt and let the thread exit naturally
                t.interrupt();
            }
        }
    }
    ...
    public synchronized void cancelChokes() {
        try {
            ...
            // Do not call Thread.stop() on the server thread.
            // Closing the listener will cause accept() to throw and the thread to exit.
            Thread serverThread = this.getServerThread();
