...
public class PeerHandler implements Runnable {
    ...
    public void run() {
        try {
            ...
            this.out.write(msg);
            this.out.flush();
            System.out.println("[DEBUG] Peer [" + this.peerAdmin.getPeerID()
                    + "] sent handshake to remote peer (unknown yet).");
            ...
            while (1 != 2) {
                if (!this.connectionEstablished) {
                    ...
                    System.out.println("[DEBUG] Peer [" + this.peerAdmin.getPeerID()
                            + "] successfully completed handshake with [" + this.endPeerID + "].");

                    if (this.peerAdmin.hasFile()
                            || this.peerAdmin.getAvailabilityOf(this.peerAdmin.getPeerID()).cardinality() > 0) {
                        this.sendBitField();
                    }
                } 
                else {
                    ...
                    if (messageType == '0') {
                        ...
                        System.out.println("[DEBUG] Peer [" + this.peerAdmin.getPeerID()
                                + "] received the 'bitfield' message from [" + this.endPeerID + "].");
                        ...
                        int pieceIndex = am.getPieceIndexFromPayload();
                        System.out.println("[DEBUG] Peer [" + this.peerAdmin.getPeerID()
                                + "] received the 'request' message from [" + this.endPeerID
                        ...
                        System.out.println("[DEBUG] Peer [" + this.peerAdmin.getPeerID()
                                + "] received the 'piece' message from [" + this.endPeerID
                                + "] for piece [" + pieceIndex + "].");
    ...
    public void sendBitField() {
        try {
            ...
            this.send(am.buildActualMessage());
            System.out.println("[DEBUG] Peer [" + this.peerAdmin.getPeerID()
                    + "] sent the 'bitfield' message to [" + this.endPeerID + "].");
        } 
        catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void sendRequestMessage(int pieceIndex) {
        try {
            ...
            this.send(am.buildActualMessage());
            System.out.println("[DEBUG] Peer [" + this.peerAdmin.getPeerID()
                    + "] sent the 'request' message to [" + this.endPeerID
                    + "] for piece [" + pieceIndex + "].");
        } 
        catch (Exception e) {
            e.printStackTrace();
    ...
    public void sendPieceMessage(int pieceIndex, byte[] payload) {
        try {
            ...
            this.send(am.buildActualMessage());
            System.out.println("[DEBUG] Peer [" + this.peerAdmin.getPeerID()
                    + "] sent the 'piece' message to [" + this.endPeerID
                    + "] for piece [" + pieceIndex + "].");
        } 
        catch (Exception e) {
            e.printStackTrace();
